...
class AdminController extends Controller {
   ...
   protected $branchIds; 
    ...
    protected function _initialize(){
		...
		define('BRANCHID',session('user_auth')['branchId']); //分院id
		$branchIds = D('Common/Branch')->getPosterityIdsCache(BRANCHID);
		$this->branchIds = isset($branchIds) && is_array($branchIds) ? $branchIds + (array)BRANCHID : (array)BRANCHID;
		array_push($this->branchIds, BRANCHID);
     ...
     * 返回数据集
     */
    protected function lists ($model,$where=array(),$order='',$base = array('status'=>array('egt',0)),$field=true, $join=''){
        $options    =   array();
        ...
        if(!empty($join)){
            return $model->field($field)->join($join)->select();
        }else {
	...
	//获得地区联级下拉菜单数据
	public function getAreaInfoByAjax($id='1') {
		$Area = new \Common\Model\AreaModel();
		$areaLists = $Area->getAreaChildren($id);
		$this->ajaxReturn($areaLists,'JSON');
