...
function upload($type=1,$sub_dir=''){
	 
	 switch ($type) {
		 case 0:
				  $_FILE = current($_FILES);
				  $file = file_get_contents($_FILE["tmp_name"]);
				  $ext = substr($_FILE["name"], strrpos($_FILE["name"], '.')+1);
				  tfs::setApi(C('TFS_UPLO0AD'));
                  $saveName = tfs::save($file, $ext, 1);
                  if ($saveName === false){
                      return ['status'=>0,'msg'=>'上传失败！'];
                  }else{
					  return ['status'=>1,'msg'=>$saveName]; 
				  }
				  break;
		 case 1: 
		         $save_dir = C('UPLOAD_DIR').'/'.$sub_dir;
				 if(!is_dir($save_dir) && @mkdir($save_dir,0777)) {
					trigger_error("$save_dir is non-existent",E_USER_NOTICE);
					return ['status'=>0,'msg'=>'上传失败！']; 
				 }
                 ...
                 if(!$info) {
                      ...
                      return ['status'=>1,'msg'=>'上传成功！', 'data'=>$info];
                 }
				 break;
		 default:
		        return false;   		  
