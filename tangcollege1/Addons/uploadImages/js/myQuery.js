/*删除上传的图片（前台删除）*/
   $(document).delegate('.btn-close', 'click', function(e) {
       ...
       var type = $(this).parents(".upload-pre-item").find("img").attr("type");
       var md5 = $(this).parents(".upload-pre-item").find("img").attr("index");
       console.log(id);
       console.log(type);
       //return false;
      for(var i = 0 ; i < $("#tab1 input[type='hidden']").length ; i++){

        if($("#tab1 input[type='hidden']").eq(i).attr('data-id') == id){
//        	var url = "{:U('File/delFile')}";
        	var url = 'index.php?s=/Admin/File/delFile';
        	$.ajax({
        		type     : 'post',
        		url      : url,
        		data     : {id : id, type : type, md5:md5},
        		dataType : 'json',
