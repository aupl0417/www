...
class StudentController extends AdminController {
	...
	public function getAddStudentTempletByAjax($classId='',$username='', $password='',$repassword='',$mobile='') {
		 if(IS_POST){
			  /* 检测密码 */
            ...
            /* 调用注册接口注册用户 */
            $User   =   new UserApi;
            $uid    =   $User->register($username, $password, null,$mobile,0,0);
            if(0 < $uid){ 
			    $data   =   I('post.');
				$data['stu_userId']  = $uid;
	...
	public function getEditStudentTempletByAjax($id='',$username='', $password='',$repassword='',$mobile='') {
		 if(empty($id)) {
			 $this->error('ID不能为空');
		 }
