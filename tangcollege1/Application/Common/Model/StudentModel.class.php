...
class StudentModel extends Model {
	...
	*/
    ...
    public function lists($where=null,$field,$order,$limit='') {
		$field = empty($field) ? 'b.*,stu_sex,stu_birthday' : $field;
        ...
        return $this->alias('a')
				   ...
				   ->join('__CLASS__ c on cl_id = cs_classId')
				   ->distinct(true)
				   ->where($where)
				   ->order($order)
				   ->field($field)
...
public function editInfo($userId,$data) {
	...
	public function cutOutStudentFromBranch($id,$branchIds) {  //从分院中剔除学生
		...
		return $this->query("delete from ".C('DB_PREFIX')."class_student where cs_classId in(select cl_id from ".C('DB_PREFIX')."class where cl_branchId in(".implode(',',$branchIds)."))");
