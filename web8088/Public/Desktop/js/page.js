/* navigator activate */
if(typeof activeI == 'number')(document.getElementById('nav-menu').getElementsByTagName('a'))[activeI].className = 'nav-active';
/**
 * index page
 */
...
window.onload = function () {
	...
	if (!fpNav) return;
    /* At the first time, we wait user for fifteen seconds and then we move to next page without patience if the user does nothing */
...
$(document).ready(function() {
    ...
    $('#fullpage').fullpage({
		...
		afterLoad: function (anchorLink, index) {
            ...
            /* when the user reach current page, we will wait for ten seconds if the user does nothing */
...
var myBox = boxController.getInstance();
document.getElementById('s-login-btn').onclick = function (event) { myBox.show(); };
