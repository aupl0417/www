...
$(function(){
	...
	$('#course_constitution').click(function () {
       if(!$('#course_local').hasClass("select_color")&&!$('#course_sort').hasClass("select_color")){
    ...
    $greet=$('#greet').attr('alt');
     ...
     function getDate(){
    	 rebtn=true;
    	 showLoading();
         var args={cate_id:cate_id,greet:$greet,area_id:diqu,nearby:$nearby,x:$x,y:$y,pages:pages};
    ...
    $('#course_local').click(function (event) {
        if(!$('#course_constitution').hasClass("select_color")&&!$('#course_sort').hasClass("select_color")) {
    ...
    $('.course_local_second_select').on('click','li',function () {
      ...
      /* 返回当前用户选择的一个地区*/
        ...
        cate_id = $('#course_constitution span').attr('alt');
        function getArea(){
            var args={area_id:area_id,cate_id:cate_id,greet:$greet,pages:pages};
        	$.post(areasUrl,args,function(msg){
        		var tpl = document.getElementById('shopinfo').innerHTML;
        		var html = juicer(tpl, msg); 
        		if(msg.info==null){
    ...
    $('#course_sort').click(function (event) {
        if(!$('#course_constitution').hasClass("select_color")&&!$('#course_local').hasClass("select_color")) {
 ...
 /*   $(window).bind('scroll', pageScroll);
    ...
    function pageScroll() {
        ...
        if ($('#commentPanel').height()-5 <= totalheight)
        {
            ...
            $(window).bind('scroll', pageScroll)
            ajax获取更多数据
            $.post(url, {'page': global[global.nowType+"Page"]}, function(data) {
