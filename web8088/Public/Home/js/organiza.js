...
$(function(){
	...
	if(sessionStorage.cateid==undefined){
		$('#course_constitution span').attr('alt','');
		 ...
		 $('#course_constitution span').attr('index',sessionStorage.parentid);
	}
	if(sessionStorage.twocatename==undefined){
		$('#course_constitution span').text("选择分类");
	}else{
		$('#course_constitution span').text(sessionStorage.twocatename);
	}
    ...
    parentid = $('#course_constitution span').attr('index');
	 ...
	 function getInfo(flat,first){
    	 ...
    	 $.post(areasUrl,args,function(msg){
    		 ...
    		 console.log(msg);
    	 		var tpl = document.getElementById('shopinfo').innerHTML;
    				var html = juicer(tpl, msg); 
    			if(msg.info==null || msg.info==false){
    				
    				$('.search_nofound').removeClass("visualization");
    				if($(".course_institution_item").size()!=0){
        				$('.search_nofound').addClass("visualization");
                        beforeAlertWinShow(myAlert);
        				 myAlert.setText("加载完毕<br/>抱歉,没有找到合适的机构");
        	        	 myAlert.show();
        	             myAlert.work(function(){
        	           	  myAlert.hidden();
                             afterAlertWinClose(myAlert);
        	                 location.href = addwishUrl;
        	             });
