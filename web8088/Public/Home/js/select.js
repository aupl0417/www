...
$(function () {
    ...
    // 判断有没有预先的cateid
    if (g.cateid && window.sessionStorage) {
        sessionStorage.setItem("cateid", g.cateid);
        var catenameMap = $.parseJSON(g.catenameMap)
        if (catenameMap[g.cateid]) {
            sessionStorage.setItem("catename", catenameMap[g.cateid]);
        }
    }
...
//        $("#cate").html('');
    ...
    if (isMobile > 800) {
        $('#cate').on('click', 'li', function (event) {
            if($(event.target).attr("alt")=="135"){ //大学活动

            	thirdcateid=$(event.target).attr("alt");
                $(".course_constitution_select_first").removeClass("course_sort_select_active");
            ...
            if($(event.target).attr("alt")=="all"){
                ...
                pages = 1;
                thirdcateid=sessionStorage.cateid;
                ajaxFlat = {};
                ajaxFlat.flat = "cate" ;
                getAllInfo(ajaxFlat.flat);
            	return;
            ...
            $.post(thirdcateUrl, args, function (msg) {
                ...
                var allnowcate='<li class="course_constitution_select_second_li" alt="'+msg["info"]["0"]["parent_id"]+'">全部</li>';
                $("#thirdcate").append(allnowcate);
    ...
    if (isMobile > 800) {
        ...
        $('.course_constitution_select_second').on('tap', '.course_constitution_select_second_li', function (event) {
    ...
    function getAllInfo(flat, first) {
        ...
        // 这里获取关键词
        var keyword = $.trim($(".register_input").val());

        if (publish == "用户发布") { //用户发布
            if (first == "nearby") {
                args = {cate_id: thirdcateid, area_id: area_id, nearby: $nearby, greet: $greet, x: $x, y: $y, pages: pages, "keyword": keyword};
                console.log(args);
            }
            else {
                args = {cate_id: thirdcateid, area_id: area_id, greet: $greet, pages: pages, "keyword": keyword};
                console.log(args);
            }
